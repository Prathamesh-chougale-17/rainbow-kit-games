  ...
  try {
    ...
    if (!gameId || !type || !walletAddress || !version) {
      return NextResponse.json(
        { error: 'Missing required fields: gameId, type, walletAddress, version' },
        { status: 400 }
      );
    }
    ...
    let result;
    if (type === 'marketplace') {
      result = await gameService.publishToMarketplace(gameId, version, walletAddress);
    } else {
      result = await gameService.publishToCommunity(gameId, version, walletAddress);
    }
