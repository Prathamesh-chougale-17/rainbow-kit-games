    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        if (errorBody.includes('NO_SCOPES_FOUND')) {
          throw new Error('Pinata JWT token lacks required file upload permissions. Please update the token with proper scopes.');
        }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Request data:', { title, walletAddress, gameId, hasHtml: !!html });

    if (!html || !title || !walletAddress) {
      console.log('Missing required fields');
      return NextResponse.json(
        { success: false, error: 'Missing required fields: html, title, walletAddress' },
        { status: 400 }
      );
    }
